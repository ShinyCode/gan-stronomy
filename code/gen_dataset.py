...
def gen_dataset(N, data_path):
    ...
    for recipe_id in recipe_ids:
        ...
        sample['class'] = classes[recipe_id]
        img = util.resize_crop_img(util.get_img_path(img_id, util.RAW_IMG_PATH))
        save_img(img_id, img, raw_img_path)
    ...
    util.save_lmdb_data(lmdb_slice, os.path.join(temp_path, 'temp.lmdb'))
