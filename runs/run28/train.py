...
def print_loss(G_BCE_loss, G_MSE_loss, D_loss, iepoch):
    print("[%s] Epoch: %d\ta*G_BCE_Loss: %f\tG_MSE_Loss: %f\tD_Loss: %f" % (util.get_time(), iepoch, opts.ALPHA * G_BCE_loss, G_MSE_loss, D_loss))
    ...
    for iepoch in range(opts.NUM_EPOCHS):
        for ibatch, data_batch in enumerate(data_loader):
            ...
            for _ in range(opts.NUM_UPDATE_G):
                ...
                G_loss = opts.ALPHA * G_BCE_loss + G_MSE_loss
                G_loss.backward()
                G_optimizer.step()
