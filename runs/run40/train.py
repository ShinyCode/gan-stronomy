    ...
    for iepoch in range(opts.NUM_EPOCHS):
        for ibatch, data_batch in enumerate(data_loader):
            ...
            if opts.NOISY_LABELS:
                noisy_real = Variable(FloatTensor(batch_size, 1).uniform_(0.9, 1.0), requires_grad=False).to(opts.DEVICE)
                noisy_fake = Variable(FloatTensor(batch_size, 1).uniform_(0.0, 0.1), requires_grad=False).to(opts.DEVICE)
